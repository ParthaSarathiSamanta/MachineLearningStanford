...
%mind it: You need to use y_matrix, which is of size m x r 

J=sum(sum(y_matrix.*log(a3)+(1-y_matrix).*log(1-a3)));
%for i=1:m
 %   for j=1:num_labels
  %      J=J+(log(a3(i,j))*y_matrix(i,j)+log(1-a3(i,j))*(1-y_matrix(i,j)));
   % end
%end
J=J/-m;
...
% a3=h needs to be used here but in case of cost(J) calculation log(h) is required
...
%regularization part for gradient
